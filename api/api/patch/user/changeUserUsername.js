    ...
    if(user){
        ...
        if(!check){
            ...
            if(check){
                ...
                res.status(200).send({ status: "USERNAME CHANGED", code: 200, action: 'DISPLAY NEW USERNAME', username: req.body.newUsername, token: check })
            }
            return res.status(401).send({status: 'USER NOT AUTHORIZED', code: 401, action: 'LOGOUT'})
        }
        
        await changeUserUsername(user._id, req.body.newUsername)
        res.status(200).send({ status: "USERNAME CHANGED", code: 200, action: 'DISPLAY NEW USERNAME', username: req.body.newUsername })
    }
