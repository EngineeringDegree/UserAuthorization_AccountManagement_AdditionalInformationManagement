...
import { getRecords, responses } from "../../actions/maps/getFields-actions"
...
const ListFieldsWrapper = () => {
    ...
    useSelector((state) => {
        if (checkIfEmptyObject(state.fields) || responses.GETTING === state.fields.response) {
            return
        }

        if (state.fields.code !== 200) {
            switch (state.fields.code) {
                case 404:
        ...
        if (loading || nameChanged) {
            ...
            setElements(state.fields.fields)
            setPage(state.fields.page)
            setPages(state.fields.pages)
        }
    })
    ...
    return (
        <div>
            ...
            <h2 className="title my-4 text-center">Fields list</h2>
                    ...
                    (error !== "") ?
                        ...
                        (elements.length === 0) ?
                            <div>
                                ...
                                <p className="orange-text text-center">No fields found</p>
                            </div>
                            :
                            <div className="control-box users-control-box">
                                <ListingMenu name="Name" page={page} setPage={setPage} pages={pages} setPages={setPages} username={name} setUsername={setName} recordPerPage={recordPerPage} setRecordsPerPage={setRecordsPerPage} />
                                <Lisiting list={elements} name={"fields"} />
...
export default connect(mapStateToProps, mapDispatchToProps)(ListFieldsWrapper)