...
function init() {
    ...
    function changeNationDeck() {
        const value = $('#my-nation-to-create-deck-from').val()
    ...
    function displayCards(cards, cardsAlreadyDisplayed) {
        let el = document.getElementById('displaying-cards')
        if (el) {
            el.innerHTML = ''
            for (let i = 0; i < cards.length; i++) {
                ...
                let cardStrength = document.createElement('input')
    ...
    function displayDeck(deck) {
        ...
        let el = document.getElementById('cards-inside')
        if (el) {
            el.innerHTML = ''
            for (let i = 0; i < deck.cards.cardsDisplayed.length; i++) {
                ...
                let qtStr = document.createElement('div')
    ...
    function editCardsFront(e) {
        const quantity = $(`#${e.target.id}`).val()
        const name = $(`#${e.target.id}-name`).text()
        const strength = $(`#${e.target.id}-strength`).val()
        let cardsPrepared = currentDecksByNations[currentDeck].cards.cardsPrepared
        let cardsDisplayed = currentDecksByNations[currentDeck].cards.cardsDisplayed
