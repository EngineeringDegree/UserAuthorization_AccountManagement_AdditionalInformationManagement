...
router.get('/', async (req, res) => {
    ...
    if (res.locals.user.data) {
        const nations = await filterAsset(req.query.records, req.query.nationName, req.query.page, Card_Nation)
        return res.status(200).send({ status: statuses.NATIONS_LISTED, code: 200, action: actions.LOGIN, token: res.locals.user.data.token, nations: nations.assets, pages: nations.pages, page: nations.page })
    }
