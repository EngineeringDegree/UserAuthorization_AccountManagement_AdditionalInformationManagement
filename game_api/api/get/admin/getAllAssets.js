...
router.get('/', async (req, res) => {
    if (res.locals.user.data) {
        ...
        return res.status(200).send({ status: statuses.ALL_RETURNED, code: 200, action: actions.LOGIN, token: res.locals.user.data.token, types: types, effects: effects, nations: nations })
    }
