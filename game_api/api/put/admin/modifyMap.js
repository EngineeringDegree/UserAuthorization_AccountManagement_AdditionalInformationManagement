...
router.put('/', async (req, res) => {
    ...
    if (res.locals.user.data) {
        ...
        if (map) {
            ...
            if (!sizeToSave) {
                return res.status(400).send({ status: statuses.BAD_SIZE_DATA, code: 404 })
            }

            const goodFields = await checkIfFieldsAreOkay(req.body.fields, req.body.size)
            if (!goodFields) {
                return res.status(400).send({ status: statuses.BAD_FIELDS, code: 404 })
            }
            ...
            if (!goodStartingPostions) {
                return res.status(400).send({ status: statuses.BAD_STARTING_POSITIONS, code: 404 })
            }
