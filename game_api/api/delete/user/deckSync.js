...
router.delete('/', async (req, res) => {
    ...
    if (res.locals.user.data) {
        ...
        if (!deck) {
            return res.status(404).send({ status: statuses.DECK_NOT_FOUND, code: 404, action: actions.GO_TO_DECKS_PAGE, token: res.locals.user.data.token })
        }
        if (deck.nation != req.body.deck.nation) {
            return res.status(401).send({ status: statuses.DECK_NATIONS_ARE_DIFFERENT, code: 401, action: actions.RELOAD, token: res.locals.user.data.token })
        }
