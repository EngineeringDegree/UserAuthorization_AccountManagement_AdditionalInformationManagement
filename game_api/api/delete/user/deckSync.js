...
router.delete('/', async (req, res) => {
    ...
    if (res.locals.user.data) {
        ...
        if (deck.nation != req.body.deck.nation) {
            return res.status(401).send({ status: 'DECK NATIONS ARE DIFFRENT', code: 401, action: actions.RELOAD, token: res.locals.user.data.token })
        }
        ...
        if (!nation) {
            return res.status(404).send({ status: statuses.NATION_NOT_FOUND, code: 404, action: 'GO TO DECKS PAGE', token: res.locals.user.data.token })
        }
        ...
        if (notSync) {
            return res.status(400).send({ status: 'NOT SYNCHRONIZED', code: 400, token: res.locals.user.data.token, action: actions.RELOAD })
        }
