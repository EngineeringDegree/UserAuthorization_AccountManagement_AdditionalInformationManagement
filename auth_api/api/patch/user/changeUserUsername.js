    ...
    if (user) {
        ...
        if (!check) {
            ...
            if (check) {
                ...
                return res.status(200).send({ status: statuses.USERNAME_CHANGED, code: 200, username: req.body.newUsername, token: check })
            }
            return res.status(401).send({ status: statuses.USER_NOT_AUTHORIZED, code: 401 })
        }

        await changeUserUsername(user._id, req.body.newUsername)
        return res.status(200).send({ status: statuses.USERNAME_CHANGED, code: 200, username: req.body.newUsername })
    }
