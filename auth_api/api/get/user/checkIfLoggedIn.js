...
router.get('/', async (req, res) => {
    ...
    if (!check) {
        ...
        return res.status(200).send({ status: statuses.USER_LOGGED_IN, code: 200, admin: user.admin, funds: user.funds, confirmed: user.confirmed, token: check, email: user.email, id: user._id })
    }

    return res.status(200).send({ status: statuses.USER_LOGGED_IN, code: 200, admin: user.admin, funds: user.funds, confirmed: user.confirmed, email: user.email, id: user._id })
})
