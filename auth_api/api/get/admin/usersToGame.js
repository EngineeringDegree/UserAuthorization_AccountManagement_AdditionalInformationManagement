...
router.get('/', async (req, res) => {
    ...
    if (req.query.gameApiSecret != process.env.GAME_API_SECRET) {
        return res.status(401).send({ status: statuses.BAD_SECRET_KEY, code: 401 })
    }
    ...
    return res.status(200).send({ status: statuses.USERS_FOUND, code: 200, user1: user1._id, user2: user2._id })
})
